...
class PrettiedNotifier(n: Notifier) {
  def asDoc: Doc = {
    ...
    Some(n.configuration)
      .filterNot(_.isEmpty)
      .foreach(c => args += assign("configuration", c.asDoc))
    Some(n.sendOnError)
      .filterNot(_ == true)
      .foreach(e => args += assign("sendOnError", e.toString))
    Some(n.sendOnFailure)
      .filterNot(_ == true)
      .foreach(e => args += assign("sendOnFailure", e.toString))
    Some(n.sendOnSuccess)
      .filterNot(_ == true)
      .foreach(e => args += assign("sendOnSuccess", e.toString))
    Some(n.sendOnWarning)
      .filterNot(_ == true)
      .foreach(e => args += assign("sendOnWarning", e.toString))
    Some(n.`type`)
      .filterNot(_ == "mail")
      .foreach(args += assignString("`type`", _))
