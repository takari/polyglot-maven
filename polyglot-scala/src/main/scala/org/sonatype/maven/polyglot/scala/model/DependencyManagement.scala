...
object DependencyManagement {
  def apply(dependencies: immutable.Seq[Dependency] = Nil) =
    new DependencyManagement(dependencies)
}
...
class PrettiedDependencyManagement(dm: DependencyManagement) {
  def asDoc: Doc = {
    ...
    Some(dm.dependencies)
      .filterNot(_.isEmpty)
      .foreach(ps => args += assign("dependencies", seq(ps.map(_.asDoc))))
...
class ConvertibleScalaDependencyManagement(dm: DependencyManagement) {
  def asJava: MavenDependencyManagement = {
    ...
    mdm.setDependencies(
      new util.ArrayList[MavenDependency](dm.dependencies.map(_.asJava).asJava)
    )
    mdm
  }
}
