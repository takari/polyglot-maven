...
case class GroupId(groupId: Option[String]) {
  ...
  def %%(artifactId: String)(implicit
...
object GroupId {
  ...
  /** Group ids are optional and when specified with just "" then they are to be
    * treated as None.
    */
...
case class GroupArtifactId(
    private val groupIdObj: GroupId,
    artifactId: String
) {
...
/** A Gav provides the coordinates of a module in a repository.
  */
case class Gav(
    private val groupArtifactIdObj: GroupArtifactId,
...
object Gav {
  ...
  /** Versions are optional and when specified with just "" then they are to be
    * treated as None.
    */
...
class PrettiedGroupArtifactId(ga: GroupArtifactId) {
  def asDoc: Doc = {
    dquotes(ga.groupId.getOrElse[String]("")) <+> percent <+> dquotes(
      ga.artifactId
    )
  }
}
