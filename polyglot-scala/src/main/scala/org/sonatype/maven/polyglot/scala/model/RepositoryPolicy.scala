...
class PrettiedRepositoryPolicy(rp: RepositoryPolicy) {
  def asDoc: Doc = {
    ...
    Option(rp.enabled)
      .filterNot(_ == true)
      .map(e => args += assign("enabled", e.toString))
    Option(rp.updatePolicy)
      .filterNot(_ == "daily")
      .map(args += assignString("updatePolicy", _))
    Option(rp.checksumPolicy)
      .filterNot(_ == "warn")
      .map(args += assignString("checksumPolicy", _))
