...
object ScalaPrettyPrinter extends PrettyPrinter {
  ...
  /** Surround `d` with a tripple-double-quote if it contains a '"' or '\n' or
    * '\n', else with a double-quote. This is named `dquotes` to shadow
    * `quotes(d: Doc): Doc` from PrettyPrinterBase.
    */
  def dquotes(d: String): Doc =
    if (d.matches(".*[\"\\n\\r].*")) surround(d, dquote <> dquote <> dquote)
    else surround(d, dquote)

  def assignString(label: String, value: String): Doc =
    label <+> equal <+> dquotes(value)
  ...
  def mapStrings(m: Map[String, String]): Doc = {
    "Map" <> lparen <> nest(
      lsep(m.map(me => dquotes(me._1) <+> "->" <+> dquotes(me._2)).toSeq, comma)
    ) <@> rparen
  }
  ...
  def seq(s: immutable.Seq[Doc]): Doc =
    "Seq" <> lparen <> nest(lsep(s, comma)) <@> rparen
  ...
  implicit def enrichPrettiedActivationFile(
      v: ActivationFile
  ): PrettiedActivationFile =
    new PrettiedActivationFile(v)

  implicit def enrichPrettiedActivationOS(
      v: ActivationOS
  ): PrettiedActivationOS =
    new PrettiedActivationOS(v)

  implicit def enrichPrettiedActivationProperty(
      v: ActivationProperty
  ): PrettiedActivationProperty =
  ...
  implicit def enrichPrettiedCiManagement(
      v: CiManagement
  ): PrettiedCiManagement =
    new PrettiedCiManagement(v)

  implicit def enrichPrettiedConfig(v: Config): PrettiedConfig =
    new PrettiedConfig(v)

  implicit def enrichPrettiedConfigurationContainer(
      v: ConfigurationContainer
  ...
  implicit def enrichPrettiedDependencyManagement(
      v: DependencyManagement
  ): PrettiedDependencyManagement =
    new PrettiedDependencyManagement(v)

  implicit def enrichPrettiedDeploymentRepository(
      v: DeploymentRepository
  ...
  implicit def enrichPrettiedDistributionManagement(
      v: DistributionManagement
  ...
  implicit def enrichPrettiedGroupArtifactId(
      v: GroupArtifactId
  ): PrettiedGroupArtifactId =
    new PrettiedGroupArtifactId(v)

  implicit def enrichPrettiedIssueManagement(
      v: IssueManagement
  ): PrettiedIssueManagement =
  ...
  implicit def enrichPrettiedOrganization(
      v: Organization
  ): PrettiedOrganization =
  ...
  implicit def enrichPrettiedPluginContainer(
      v: PluginContainer
  ): PrettiedPluginContainer =
    new PrettiedPluginContainer(v)

  implicit def enrichPrettiedPluginManagement(
      v: PluginManagement
  ): PrettiedPluginManagement =
    new PrettiedPluginManagement(v)

  implicit def enrichPrettiedPrerequisites(
      v: Prerequisites
  ): PrettiedPrerequisites =
    new PrettiedPrerequisites(v)

  implicit def enrichPrettiedProfile(v: Profile): PrettiedProfile =
    new PrettiedProfile(v)

  implicit def enrichPrettiedProperties(
      v: Map[String, String]
  ): PrettiedProperties =
  ...
  implicit def enrichPrettiedRepositoryPolicy(
      v: RepositoryPolicy
  ): PrettiedRepositoryPolicy =
  ...
  implicit def enrichPrettiedReportPlugin(
      v: ReportPlugin
  ): PrettiedReportPlugin =
...
/** implicit conversions around the "pimp my library" approach for converting
  * Maven models to their Scala types.
  */
object MavenConverters {
  implicit def enrichConvertibleActivation(
      v: MavenActivation
  ): ConvertibleMavenActivation =
    new ConvertibleMavenActivation(v)

  implicit def enrichConvertibleActivationFile(
      v: MavenActivationFile
  ): ConvertibleMavenActivationFile =
    new ConvertibleMavenActivationFile(v)

  implicit def enrichConvertibleActivationOS(
      v: MavenActivationOS
  ): ConvertibleMavenActivationOS =
    new ConvertibleMavenActivationOS(v)

  implicit def enrichConvertibleActivationProperty(
      v: MavenActivationProperty
  ...
  implicit def enrichConvertibleBuildBase(
      v: MavenBuildBase
  ): ConvertibleMavenBuildBase =
    new ConvertibleMavenBuildBase(v)

  implicit def enrichConvertibleCiManagement(
      v: MavenCiManagement
  ): ConvertibleMavenCiManagement =
  ...
  implicit def enrichConvertibleContributor(
      v: MavenContributor
  ): ConvertibleMavenContributor =
    new ConvertibleMavenContributor(v)

  implicit def enrichConvertibleDependency(
      v: MavenDependency
  ): ConvertibleMavenDependency =
    new ConvertibleMavenDependency(v)

  implicit def enrichConvertibleDependencyManagement(
      v: MavenDependencyManagement
  ): ConvertibleMavenDependencyManagement =
    new ConvertibleMavenDependencyManagement(v)

  implicit def enrichConvertibleDeploymentRepository(
      v: MavenDeploymentRepository
  ): ConvertibleMavenDeploymentRepository =
    new ConvertibleMavenDeploymentRepository(v)

  implicit def enrichConvertibleDeveloper(
      v: MavenDeveloper
  ): ConvertibleMavenDeveloper =
    new ConvertibleMavenDeveloper(v)

  implicit def enrichConvertibleDistributionManagement(
      v: MavenDistributionManagement
  ): ConvertibleMavenDistributionManagement =
    new ConvertibleMavenDistributionManagement(v)

  implicit def enrichConvertibleExecution(
      v: MavenExecution
  ): ConvertibleMavenExecution =
    new ConvertibleMavenExecution(v)

  implicit def enrichConvertibleExtension(
      v: MavenExtension
  ): ConvertibleMavenExtension =
    new ConvertibleMavenExtension(v)

  implicit def enrichConvertibleGav(
      v: (String, String, String)
  ): ConvertibleMavenGav =
    new ConvertibleMavenGav(v)

  implicit def enrichConvertibleGroupArtifactId(
      v: (String, String)
  ): ConvertibleMavenGroupArtifactId =
    new ConvertibleMavenGroupArtifactId(v)

  implicit def enrichConvertibleIssueManagement(
      v: MavenIssueManagement
  ): ConvertibleMavenIssueManagement =
    new ConvertibleMavenIssueManagement(v)

  implicit def enrichConvertibleLicense(
      v: MavenLicense
  ): ConvertibleMavenLicense =
    new ConvertibleMavenLicense(v)

  implicit def enrichConvertibleMailingList(
      v: MavenMailingList
  ): ConvertibleMavenMailingList =
  ...
  implicit def enrichConvertibleNotifier(
      v: MavenNotifier
  ): ConvertibleMavenNotifier =
    new ConvertibleMavenNotifier(v)

  implicit def enrichConvertibleOrganization(
      v: MavenOrganization
  ): ConvertibleMavenOrganization =
  ...
  implicit def enrichConvertiblePluginManagement(
      v: MavenPluginManagement
  ): ConvertibleMavenPluginManagement =
    new ConvertibleMavenPluginManagement(v)

  implicit def enrichConvertiblePrerequisites(
      v: MavenPrerequisites
  ): ConvertibleMavenPrerequisites =
    new ConvertibleMavenPrerequisites(v)

  implicit def enrichConvertibleProfile(
      v: MavenProfile
  ): ConvertibleMavenProfile =
    new ConvertibleMavenProfile(v)

  implicit def enrichConvertibleRelocation(
      v: MavenRelocation
  ): ConvertibleMavenRelocation =
    new ConvertibleMavenRelocation(v)

  implicit def enrichConvertibleRepositoryPolicy(
      v: MavenRepositoryPolicy
  ): ConvertibleMavenRepositoryPolicy =
    new ConvertibleMavenRepositoryPolicy(v)

  implicit def enrichConvertibleRepository(
      v: MavenRepository
  ): ConvertibleMavenRepository =
    new ConvertibleMavenRepository(v)

  implicit def enrichConvertibleResource(
      v: MavenResource
  ): ConvertibleMavenResource =
  ...
  implicit def enrichConvertibleReporting(
      v: MavenReporting
  ): ConvertibleMavenReporting =
    new ConvertibleMavenReporting(v)

  implicit def enrichConvertibleReportPlugin(
      v: MavenReportPlugin
  ): ConvertibleMavenReportPlugin =
    new ConvertibleMavenReportPlugin(v)

  implicit def enrichConvertibleReportSet(
      v: MavenReportSet
  ): ConvertibleMavenReportSet =
