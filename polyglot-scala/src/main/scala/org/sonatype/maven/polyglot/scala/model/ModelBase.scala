...
class PrettiedModelBase(m: ModelBase) {
  def asDocArgs: immutable.Seq[Doc] = {
    ...
    Some(m.modules)
      .filterNot(_.isEmpty)
      .foreach(m => args += assign("modules", seqString(m)))
    Some(m.repositories)
      .filterNot(_.isEmpty)
      .foreach(es => args += assign("repositories", seq(es.map(_.asDoc))))
    Some(m.pluginRepositories)
      .filterNot(_.isEmpty)
      .foreach(es => args += assign("pluginRepositories", seq(es.map(_.asDoc))))
    Some(m.dependencies)
      .filterNot(_.isEmpty)
      .foreach(es => args += assign("dependencies", seq(es.map(_.asDoc))))
    m.dependencyManagement.foreach(dm =>
      args += assign("dependencyManagement", dm.asDoc)
    )
    m.distributionManagement.foreach(dm =>
      args += assign("distributionManagement", dm.asDoc)
    )
    args.toList
  }
}
