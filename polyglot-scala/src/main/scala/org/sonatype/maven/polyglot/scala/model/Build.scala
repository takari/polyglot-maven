...
class PrettiedBuild(b: Build) {
  def asDoc: Doc = {
    ...
    b.scriptSourceDirectory.foreach(
      args += assignString("scriptSourceDirectory", _)
    )
    b.testSourceDirectory.foreach(
      args += assignString("testSourceDirectory", _)
    )
    b.outputDirectory.foreach(args += assignString("outputDirectory", _))
    b.testOutputDirectory.foreach(
      args += assignString("testOutputDirectory", _)
    )
    Some(b.extensions)
      .filterNot(_.isEmpty)
      .foreach(es => args += assign("extensions", seq(es.map(_.asDoc))))
    args ++= b.asDocArgs
