...
/** A Config object lets us conveniently express objects that can have any
  * property with any type. This generally serves as configuration to a plugin.
  *
  ...
  * }}}
  *
  * The above sample semantically resembles the following Maven `pom.xml`
  * snippet:
  *
  * {{{
  * <configuration>
...
class Config(val elements: immutable.Seq[(String, Option[Any])]) {
  ...
  /** Returns a new Config with contains the elements of this and the other
    * config.
    */
...
object Config extends Dynamic {
  ...
  /** Handle the missmatch of legal characters in scala method names vs. XML
    * elements
    */
  ...
  def sanitizeElementName(k: String): String = {
    val r = elementStartCharMapping.foldLeft(k)((k, m) =>
  ...
  private val config = mc match {
    case xmlConfig: Xpp3Dom =>
      def asConfig(xmlConfig: Xpp3Dom): Config = {
        ...
        xmlConfig.getAttributeNames.foreach { attrName =>
