...
/** implicit conversions around the "pimp my library" approach for converting
  * Scala models to their Maven types.
  */
object ScalaConverters {
  implicit def enrichScalaActivation(
      v: ScalaActivation
  ): ConvertibleScalaActivation =
    new ConvertibleScalaActivation(v)

  implicit def enrichScalaActivationFile(
      v: ScalaActivationFile
  ): ConvertibleScalaActivationFile =
    new ConvertibleScalaActivationFile(v)

  implicit def enrichScalaActivationOS(
      v: ScalaActivationOS
  ): ConvertibleScalaActivationOS =
    new ConvertibleScalaActivationOS(v)

  implicit def enrichScalaActivationProperty(
      v: ScalaActivationProperty
  ...
  implicit def enrichScalaBuildBase(
      v: ScalaBuildBase
  ): ConvertibleScalaBuildBase =
    new ConvertibleScalaBuildBase(v)

  implicit def enrichScalaCiManagement(
      v: ScalaCiManagement
  ): ConvertibleScalaCiManagement =
  ...
  implicit def enrichScalaContributor(
      v: ScalaContributor
  ): ConvertibleScalaContributor =
    new ConvertibleScalaContributor(v)

  implicit def enrichScalaDependency(
      v: ScalaDependency
  ): ConvertibleScalaDependency =
    new ConvertibleScalaDependency(v)

  implicit def enrichScalaDependencyManagement(
      v: ScalaDependencyManagement
  ): ConvertibleScalaDependencyManagement =
    new ConvertibleScalaDependencyManagement(v)

  implicit def enrichScalaDeploymentRepository(
      v: ScalaDeploymentRepository
  ): ConvertibleScalaDeploymentRepository =
    new ConvertibleScalaDeploymentRepository(v)

  implicit def enrichScalaDeveloper(
      v: ScalaDeveloper
  ): ConvertibleScalaDeveloper =
    new ConvertibleScalaDeveloper(v)

  implicit def enrichScalaDistributionManagement(
      v: ScalaDistributionManagement
  ): ConvertibleScalaDistributionManagement =
    new ConvertibleScalaDistributionManagement(v)

  implicit def enrichScalaExecution(
      v: ScalaExecution
  ): ConvertibleScalaExecution =
    new ConvertibleScalaExecution(v)

  implicit def enrichScalaExtension(
      v: ScalaExtension
  ): ConvertibleScalaExtension =
    new ConvertibleScalaExtension(v)

  implicit def enrichScalaIssueManagement(
      v: ScalaIssueManagement
  ...
  implicit def enrichScalaMailingList(
      v: ScalaMailingList
  ): ConvertibleScalaMailingList =
  ...
  implicit def enrichScalaOrganization(
      v: ScalaOrganization
  ): ConvertibleScalaOrganization =
  ...
  implicit def enrichScalaPluginManagement(
      v: ScalaPluginManagement
  ): ConvertibleScalaPluginManagement =
    new ConvertibleScalaPluginManagement(v)

  implicit def enrichScalaPrerequisites(
      v: Prerequisites
  ): ConvertibleScalaPrerequisites =
  ...
  implicit def enrichScalaReleasePolicy(
      v: ScalaRepositoryPolicy
  ): ConvertibleScalaRepositoryPolicy =
    new ConvertibleScalaRepositoryPolicy(v)

  implicit def enrichScalaRelocation(
      v: ScalaRelocation
  ): ConvertibleScalaRelocation =
    new ConvertibleScalaRelocation(v)

  implicit def enrichScalaRepository(
      v: ScalaRepository
  ): ConvertibleScalaRepository =
  ...
  implicit def enrichScalaReportPlugin(
      v: ReportPlugin
  ): ConvertibleScalaReportPlugin =
...
/** Reads a Scala model from a script and produces a Maven Model object. All
  * Scala evaluation is done with files so that checks can be made to determine
  * whether compilation occurs.
  */
@Named("scala")
class ScalaModelReader @Inject() (executeManager: ExecuteManager)
    extends ModelReader {
  ...
  private def locateEvalPomFile(options: util.Map[String, _]): File = {
    val targetDir =
      Option(System.getProperty("polyglot.scala.outputdir")).getOrElse("target")
    val source = PolyglotModelUtil.getLocation(options)
    val binVersion = _root_.scala.util.Properties.versionNumberString
      .split("[.]")
      .take(2)
      .mkString(".")
    val evalTarget = new File(
      new File(source).getParent,
      targetDir + File.separator + "scalamodel_" + binVersion
    )
    evalTarget.mkdirs()
    new File(evalTarget, "pom.scala")
  }

  /** We subclass the [[Eval]] class to customize the otherwise immutable
    * prepocessors property.
    *
    * We provide an [[MvnIncludePreprocessor]] that resolves files and classes
    * from an (externally) defined directory and the the current classloader.
    */
  class MvnEval(target: Option[File], includeBaseDir: File)
      extends Eval(target) {
     ...
     */
    class MvnIncludePreprocessor(resolvers: Seq[Resolver])
  ...
  private def eval(
    ...
    try {
      ...
      case e: Throwable =>
        throw new ModelParseException(
          "Could not process pom file: " + sourceFile,
          0,
          0,
          e
        )
  ...
  private def registerExecutors(
    ...
    executeManager.register(
      m,
      tasks.map(new ScalaTask(_).asInstanceOf[ExecuteTask]).asJava
    )
