...
class PrettiedBuildBase(b: BuildBase) {
  ...
  def asDocArgs: immutable.Seq[Doc] = {
    ...
    Some(b.resources)
      .filterNot(_.isEmpty)
      .foreach(r => args += assign("resources", seq(r.map(_.asDoc))))
    Some(b.testResources)
      .filterNot(_.isEmpty)
      .foreach(trs => args += assign("testResources", seq(trs.map(_.asDoc))))
    b.directory.foreach(args += assignString("directory", _))
    b.finalName.foreach(args += assignString("finalName", _))
    Some(b.filters)
      .filterNot(_.isEmpty)
      .foreach(f => args += assign("filters", seqString(f)))
    b.pluginManagement.foreach(pm =>
      args += assign("pluginManagement", pm.asDoc)
    )
    Some(b.plugins)
      .filterNot(_.isEmpty)
      .foreach(ps => args += assign("plugins", seq(ps.map(_.asDoc))))
    Some(b.tasks)
      .filterNot(_.isEmpty)
      .foreach(ps => args += assign("tasks", seq(ps.map(_.asDoc))))
    args.toList
  }
}
