...
public interface BuildTemplate extends PluginMethods {
	...
	public default Resource resource(String directory, String targetPath, boolean filtering, String[] includes, String[] excludes) {
		return resource(directory, targetPath, filtering, Arrays.asList(includes), Arrays.asList(excludes));
	}
	
	public default Resource resource(String directory, String targetPath, boolean filtering, List<String> includes, List<String> excludes) {
	...
	public default Resource resource(ResourcesNamedValue... keyValuePairs) {
		...
		asList(keyValuePairs).stream().filter(kvp -> kvp != null).filter(kvp -> !(kvp instanceof ResourcesIncludesNamedValue) && !(kvp instanceof ResourcesExcludesNamedValue))
		.forEach(kvp -> map.put(kvp.name(), kvp.value()));
		NamedValueProcessor.mapToObject(resource, map);
		
		asList(keyValuePairs).stream().filter(kvp -> kvp != null).filter(kvp -> (kvp instanceof ResourcesIncludesNamedValue))
		.forEach(kvp -> resource.setIncludes(((ResourcesIncludesNamedValue)kvp).getIncludes()));

		asList(keyValuePairs).stream().filter(kvp -> kvp != null).filter(kvp -> (kvp instanceof ResourcesExcludesNamedValue))
